...
image_to_1bit_pixmap(image_t *image, xcb_drawable_t d)
{
    ...
    gc = xcb_generate_id(_G_connection);
    xcb_create_gc(_G_connection, gc, pixmap, 0, NULL);
    ...
    xcb_image_put(_G_connection, pixmap, gc, img, 0, 0, 0);

    xcb_free_gc(_G_connection, gc);
